...
def half_time_computation(save_data=True):
    ...
    Np_all = np.array([1.0])
    Nd_all = np.linspace(0, 10**4, 5000)
    Half_life_for_Np = []
    for Np in Np_all:
        ...
        if save_data:
            dirname = "../Data/"
            fname = "Half_times_data_for_Np_%d.h5" % Np
            hf = h5py.File(dirname + fname, 'w')
            hf.create_dataset('Nd', data=Nd_good)
            hf.create_dataset('Avg_half_times', data=avg_half_times)
            hf.close()
